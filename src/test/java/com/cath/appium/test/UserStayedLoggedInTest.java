...
public class UserStayedLoggedInTest extends TestCase {
    ...
    public void loginAfterKill()  {

        logManager.loggingInfo("Killing the app");
        DriverManager.getInstance().getAndroidDriver().closeApp();

        logManager.loggingInfo("Opening the app again");
        DriverManager.getInstance().getAndroidDriver().launchApp();
